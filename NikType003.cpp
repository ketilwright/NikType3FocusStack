...
void NikType003::OnEvent(const NKEvent* ev)
{
    ...
    if(PTP_EC_CaptureComplete == ev->eventCode )
    {
        ...
        g_print->print(F("Capture complete"));
        m_stateFlags.m_captureInProgress = 0;
        if(m_stateFlags.m_stackActive)
        {
            focusStackNextFrame();
        }
    }
...
uint16_t NikType003::captureToCard()
{
    ...
    if(!m_stateFlags.m_captureInProgress)
    {
        ...
        ret = Operation(PTP_OC_NIKON_CaptureRecInMedia, 2, params);
        m_stateFlags.m_captureInProgress = (PTP_RC_OK == ret);
    }
    return ret;
}
...
uint16_t NikType003::startFocusStack()
{
    ...
    if(PTP_RC_OK == ret)
    {
        m_remainingFrames--;
        g_print->clear();
        g_print->print(F("Frames remaining ")); g_print->print(m_remainingFrames);
    }
    else
    {
        m_remainingFrames = 0;
...
// drives focus for the next frame, waits
uint16_t NikType003::focusStackNextFrame()
{
    ...
    if(m_remainingFrames > 0)
    {
        ...
        switch(retVal)
        {
            case PTP_RC_OK:
            {
                g_print->print(F("Remaining: ")); g_print->print(m_remainingFrames);
                break;
            }
            case NK_RC_MfDriveStepInsufficiency:
            {
                g_print->print(F("Infinite focus"));
                break;
            }
            case PTP_RC_StoreFull:
            {
                g_print->print(F("Card full"));
                break;
            }
        }
    }
